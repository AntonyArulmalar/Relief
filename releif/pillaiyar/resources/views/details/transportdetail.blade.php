    ...
    <div class="row form-group">
        <div class="col-xs-12">
            <ul class="nav nav-pills nav-justified thumbnail setup-panel">
                ...
                <li class="active"><a href="/details/transportdetail">
...
<tr>
        ...
        @foreach($transports as $retrieve)
    <tr>
        
        
            <td> {{$retrieve->ownername}}</td>
            <td> {{$retrieve->ownercontact}}</td>
            <td> {{$retrieve->center}}</td>
            <td> {{$retrieve->transporttype}}</td>
            <td> {{$retrieve->availability}}</td>
            <td> {{$retrieve->vehicleno}}</td>
            <td> {{$retrieve->vehiclelocation}}</td>
            <td> {{$retrieve->date}}</td>
            <td> {{$retrieve->capacity}}</td>
            <td> <a href={{"/details/transportdetail/delete/{id} , $retrieve->id"}} >Delete</a> </td>
...
<a id="add_row" class="btn btn-success pull-left" href="/forms/transport/submit">Add More Transport</a>
</div>
</div>
</div>
